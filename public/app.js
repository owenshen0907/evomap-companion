...
const I18N = {
  ...
  ja: {
    ...
    valueEyebrow: 'Why EvoMap', valueTitle: 'EvoMap で何が省ける', valueMechanism: '検証済みの Gene / Capsule を再利用し、ゼロからの探索と試行錯誤を省く — トークン・時間・クレジットを節約。再利用するほど省ける。', valueStep1: '「統合」で Claude Code / Cursor / Codex に統合し、リコールライブラリを IDE に書き込む。', valueStep2: '「資産検索」でトピック検索し、関連資産をローカルに fetch。', valueStep3: 'IDE を再起動し、下の一文を agent に貼り、毎タスク開始時にまず recall させる。', valuePromptLabel: 'agent にコピー：', valuePrompt: 'タスク開始前に、IDE のグローバル設定内の EvoMap RECALL.md（例 ~/.claude/evomap/RECALL.md）を読み、キャッシュ済み資産を recall。一致する戦略や修正があれば asset_id を明記して再利用し、ゼロから解かずトークンを節約する。さらに必要なら http://localhost:4174/api/assets/discover?mode=semantic&q=<トピック> を呼ぶ。', copyPrompt: 'コピー', copied: 'コピーしました', pubPromptLabel: '再利用可能なタスクを終えたら、これを agent にコピー：', pubPrompt: '再利用可能なタスクが完了しました。EvoMap 資産の下書きにまとめてください：Gene（再利用可能な戦略）か Capsule（具体的な成果）かを判断し、タイトル・要約・主要手順やコード・タグを抽出し、POST http://localhost:4174/api/publish/draft で下書きを作成、アップロード内容を確認させてください。鍵・プライベートパス・機密情報は含めず、確認後にのみアップロード。',
